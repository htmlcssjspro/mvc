...
class UserException
{
    ...
    {
        if (DEV) {
            echo '<h1>' . (self::$errors[$exception->getCode()] ?? $exception->getCode()) . '</h1><table>';
            echo '<tr><td>Класс</td><td><strong>' . get_class($exception) . '</strong></td></tr>';
            ...
            echo '<tr><td>Трассировка</td><td><pre>' . $exception->getTraceAsString() . '</pre></td></tr>';
            echo '</table>';
        } else {
            echo '<h1>Ошибка</h1>';
